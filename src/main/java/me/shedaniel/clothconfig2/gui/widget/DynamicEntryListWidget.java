...
public abstract class DynamicEntryListWidget<E extends DynamicEntryListWidget.Entry<E>> extends AbstractParentElement implements Drawable, Selectable {
    ...
    public SelectionType getType() {
        if (this.isFocused()) {
            ...
            return this.hoveredItem != null ? SelectionType.HOVERED : SelectionType.NONE;
        }
    }
    ...
    public void appendNarrations(NarrationMessageBuilder narrationElementOutput) {
        ...
        if (entry != null) {
            entry.updateNarration(narrationElementOutput.nextMessage());
            this.narrateListElementPosition(narrationElementOutput, entry);
        } else {
            E entry2 = this.getFocused();
            if (entry2 != null) {
                entry2.updateNarration(narrationElementOutput.nextMessage());
    ...
    protected void narrateListElementPosition(NarrationMessageBuilder narrationElementOutput, E entry) {
        List<E> list = this.visibleChildren();
        if (list.size() > 1) {
            int i = list.indexOf(entry);
            if (i != -1) {
                narrationElementOutput.put(NarrationPart.POSITION, Text.translatable("narrator.position.list", i + 1, list.size()));
            }
        }
    }
    ...
    protected void renderHeader(DrawContext graphics, int rowLeft, int startY, Tessellator tessellator) {
    }
    
    protected void drawBackground() {
    }
    
    protected void renderDecorations(DrawContext graphics, int mouseX, int mouseY) {
    }
    ...
    @Nullable
    protected E nextEntry(NavigationDirection screenDirection) {
    ...
    @Nullable
    protected E nextEntry(NavigationDirection screenDirection, Predicate<E> predicate) {
        return this.nextEntry(screenDirection, predicate, this.getSelectedItem());
    }
    
    @Nullable
    protected E nextEntry(NavigationDirection screenDirection, Predicate<E> predicate, @Nullable E entry) {
    ...
    protected void moveSelection(int shift) {
        ...
        int index = MathHelper.clamp(selected + shift, 0, this.getItemCount() - 1);
    ...
    protected void renderItem(DrawContext graphics, E item, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isSelected, float delta) {
    ...
    @Environment(EnvType.CLIENT)
    public abstract static class Entry<E extends Entry<E>> implements Element, TickableWidget, HideableWidget, DisableableWidget {
        ...
        public abstract void render(DrawContext graphics, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isHovered, float delta);
        ...
        void updateNarration(NarrationMessageBuilder narrationElementOutput) {
