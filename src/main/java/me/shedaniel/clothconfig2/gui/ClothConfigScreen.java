...
public class ClothConfigScreen extends AbstractTabbedConfigScreen {
    ...
    private ClickableWidget buttonLeftTab, buttonRightTab;
    ...
    @ApiStatus.Internal
    public ClothConfigScreen(Screen parent, Text title, Map<String, ConfigCategory> categoryMap, Identifier backgroundLocation) {
    ...
    protected void init() {
        ...
        childrenL().add(listWidget = new ListWidget(this, client, width, height, isShowingTabs() ? 70 : 30, height - 32, getBackgroundLocation()));
        ...
        addDrawableChild(ButtonWidget.builder(isEdited() ? Text.translatable("text.cloth-config.cancel_discard") : Text.translatable("gui.cancel"), widget -> quit()).dimensions(width / 2 - buttonWidths - 3, height - 26, buttonWidths, 20).build());
        addDrawableChild(new ButtonWidget(width / 2 + 3, height - 26, buttonWidths, 20, Text.empty(), button -> saveAll(true), Supplier::get) {
            @Override
        ...
        if (isShowingTabs()) {
            ...
            childrenL().add(buttonLeftTab = new ButtonWidget(4, 44, 12, 18, Text.empty(), button -> tabsScroller.scrollTo(0, true), Supplier::get) {
                @Override
                public void renderButton(DrawContext graphics, int mouseX, int mouseY, float delta) {
                    ...
                    graphics.drawTexture(CONFIG_TEX, getX(), getY(), 12, 18 * (!this.isNarratable() ? 0 : this.isSelected() ? 2 : 1), width, height);
                }
            });
            int j = 0;
            for (Pair<Text, Integer> tab : tabs) {
                tabButtons.add(new ClothConfigTabButton(this, j, -100, 43, tab.getRight(), 20, tab.getLeft(), this.categoryMap.get(tab.getLeft().getString()).getDescription()));
                j++;
            }
            childrenL().addAll(tabButtons);
            childrenL().add(buttonRightTab = new ButtonWidget(width - 16, 44, 12, 18, Text.empty(), button -> tabsScroller.scrollTo(tabsScroller.getMaxScroll(), true), Supplier::get) {
                @Override
                public void renderButton(DrawContext graphics, int mouseX, int mouseY, float delta) {
                    RenderSystem.setShader(GameRenderer::getPositionTexProgram);
                    RenderSystem.setShaderTexture(0, CONFIG_TEX);
                    ...
                    RenderSystem.blendFuncSeparate(770, 771, 0, 1);
                    RenderSystem.blendFunc(770, 771);
                    graphics.drawTexture(CONFIG_TEX, getX(), getY(), 0, 18 * (!this.isNarratable() ? 0 : this.isSelected() ? 2 : 1), width, height);
                }
            });
        } else {
    ...
    public void render(DrawContext graphics, int mouseX, int mouseY, float delta) {
        ...
        if (isShowingTabs()) {
            graphics.drawCenteredTextWithShadow(client.textRenderer, title, width / 2, 18, -1);
            ...
            graphics.drawCenteredTextWithShadow(client.textRenderer, title, width / 2, 12, -1);
        ...
        if (isEditable()) {
            ...
            if (errors.size() > 0) {
                ...
                String text = "§c" + (errors.size() == 1 ? errors.get(0).copyContentOnly().getString() : I18n.translate("text.cloth-config.multi_error"));
                if (isTransparentBackground()) {
                    int stringWidth = client.textRenderer.getWidth(text);
                    graphics.fillGradient(8, 9, 20 + stringWidth, 14 + client.textRenderer.fontHeight, 0x68000000, 0x68000000);
                }
                graphics.drawTexture(CONFIG_TEX, 10, 10, 0, 54, 3, 11);
                graphics.drawTextWithShadow(client.textRenderer, text, 18, 12, -1);
                if (errors.size() > 1) {
                    int stringWidth = client.textRenderer.getWidth(text);
    ...
    private void drawTabsShades(DrawContext graphics, int lightColor, int darkColor) {
        drawTabsShades(graphics.getMatrices(), lightColor, darkColor);
    }
    
    private void drawTabsShades(MatrixStack matrices, int lightColor, int darkColor) {
        drawTabsShades(matrices.peek().getPositionMatrix(), lightColor, darkColor);
    }
    ...
    public static class ListWidget<R extends DynamicElementListWidget.ElementEntry<R>> extends DynamicElementListWidget<R> {
        ...
        @Override
        protected void renderItem(DrawContext graphics, R item, int index, int y, int x, int entryWidth, int entryHeight, int mouseX, int mouseY, boolean isSelected, float delta) {
        ...
        @Override
        protected void renderList(DrawContext graphics, int startX, int startY, int mouseX, int mouseY, float delta) {
            thisTimeTarget = null;
        ...
        @Override
        protected void renderBackBackground(DrawContext graphics, BufferBuilder buffer, Tessellator tessellator) {
        ...
        @Override
        protected void renderHoleBackground(DrawContext graphics, int y1, int y2, int alpha1, int alpha2) {
            if (!screen.isTransparentBackground())
